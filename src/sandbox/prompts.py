...
# Batched sibling of build_perception_prompt: turns a WEEK's worth of
# same-kind messages (currently used for tweets only — see engine.py) into
# distinct, deduplicated takeaway memories in one call, VacSim-style
# (feed_tweets() -> generate_and_save_lessons()), instead of one call per
# message. Each memory is tagged with the message number(s) it came from so
# the caller can resolve provenance (source_agent_id) back to the original
# message(s).
BATCH_PERCEPTION_SYSTEM = """You are simulating how a Singapore resident
internalises several things they just read this week. Given their profile
and a numbered list of incoming messages, write the DISTINCT first-person
takeaway memories this person forms — one sentence each, filtered through
their own circumstances. If two or more messages would produce the same or a
very similar takeaway, output ONE combined memory rather than repeating it,
and list every message number it draws from; otherwise list just the one
number it came from. Also estimate each memory's importance to them on a 0-1
scale (0 = barely memorable, 1 = life-changing).

Respond with JSON only:
{"memories": [{"memory_text": "...", "importance": 0.5, "source_indices": [1]}, ...]}"""


def build_batch_perception_prompt(agent, numbered_messages, message_kind):
    """(system, user) prompt turning a week's worth of same-kind messages into
    distinct, deduplicated first-person takeaway memories.

    `numbered_messages` is [(1, text), (2, text), ...], 1-indexed, matching
    the order callers must map each memory's `source_indices` back against.
    `message_kind` is a human label for the source ("social media posts") so
    the prompt reads naturally.
    """
    lines = [f"{i}. \"{text}\"" for i, text in numbered_messages]
    user = (
        "Profile of the person:\n"
        f"{profile_to_str(agent)}\n\n"
        f"They just read these {message_kind} this week:\n" + "\n".join(lines) + "\n\n"
        "Write their distinct first-person takeaway memories as JSON."
    )
    return BATCH_PERCEPTION_SYSTEM, user
