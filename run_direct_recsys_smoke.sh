#!/bin/bash
# Smoke-test the VacSim-recsys variant (driver_direct_recsys.py /
# engines/engine_direct_recsys.py) across all four conditions, on a small
# slice of agents/weeks. Same seeded agents, news/context corpus, and social
# network as run_direct_smoke.sh -- the only difference from that script is
# that news/tweet EXPOSURE is recommender-selected per agent (embedding
# similarity, ported from ~/VacSim's src/recommenders/) instead of a fixed
# weekly broadcast / plain follow-list. See
# engines/engine_direct_recsys.py's module docstring for the full comparison.
#
# Usage:
#   tmux new -s fark_recsys_smoke
#   bash run_direct_recsys_smoke.sh
#   # detach: Ctrl-b d ; reattach later: tmux attach -t fark_recsys_smoke
#
...
# outputs/runs/smoke/, namespaced smoke_recsys_C0..C3 so they never collide
# with the TPB study, the fixed-schedule direct smoke runs, or a full-scale
# run_C0..C3_recsys.
#
...
# Also requires sentence-transformers (already a project dependency, used for
# cosine/hybrid TPB relevance) -- the recommenders download
# all-MiniLM-L6-v2 on first use if not already cached.
...
# C0 — static baseline: no policy, no social. Recommenders never run.
run smoke_recsys_C0 --condition C0

# C1 — social only: exercises TweetRecommender (similarity + follow-boost).
run smoke_recsys_C1 --condition C1 --network "$NETWORK"

# C2 — policy only: exercises NewsRecommender against the growing article pool.
run smoke_recsys_C2 --condition C2 --news-corpus "$NEWS_CORPUS" --policy-category combined

# C3 — policy + social: exercises both recommenders together.
run smoke_recsys_C3 --condition C3 --network "$NETWORK" --news-corpus "$NEWS_CORPUS"

echo
echo "All 4 smoke runs complete. Outputs in $RUNS_DIR/{smoke_recsys_C0,smoke_recsys_C1,smoke_recsys_C2,smoke_recsys_C3}.json"
